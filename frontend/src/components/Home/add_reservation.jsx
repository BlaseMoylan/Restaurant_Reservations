...
const AddReservation = ({getWaitList,waitList,getReservations,usedTables,allTables,getAllTables,getUsedTables}) => {
  ...
  async function addReservation(form) {
    try {
      let results = await axios.post(`http://35.87.21.157:8000/api/user_reservations`,form,
        {
          headers: {
  ...
  async function postUsedTable(form) {
    let results = await axios.post(
      `http://35.87.21.157:8000/api/used_tables`,
      form
    );
    getUsedTables()
  }
  ...
  async function getUnavailable(){
    let results= await axios.get(`http://35.87.21.157:8000/api/unavailable`)
    setUnavailable(results.data)
  }
  async function postUnavailable(form){
    let results= await axios.post(`http://35.87.21.157:8000/api/unavailable`,form)
    getUnavailable()
  }

  async function postWaitList(form){
    let results= await axios.post(`http://35.87.21.157:8000/api/wait_list`,form)
    getWaitList()
  }
