...
const HomePage = () => {
  ...
  async function getWaitList(){
    let results= await axios.get(`http://${IP}:8000/api/wait_list`)
    setWaitList(results.data)
  }

  async function getReservations(){
    let results= await axios.get(`http://${IP}:8000/api/all_reservations`)
    setReservations(results.data)
    
  }

  async function getUsedTables() {
    let results = await axios.get(`http://${IP}:8000/api/used_tables`);
  ...
  async function getAllTables() {
    let results = await axios.get(`http://${IP}:8000/api/all_tables`);
